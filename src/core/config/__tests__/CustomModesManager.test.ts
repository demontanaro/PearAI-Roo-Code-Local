...
describe("CustomModesManager", () => {
	...
	const mockRoomodes = `${path.sep}mock${path.sep}workspace${path.sep}.pearai-agent-modes`
	...
	describe("getCustomModes", () => {
		it("should merge modes with .pearai-agent-modes taking precedence", async () => {
			const settingsModes = [
			...
			// mode2 should come from .pearai-agent-modes since it takes precedence
		...
		it("should handle missing .pearai-agent-modes file", async () => {
		...
		it("should handle invalid JSON in .pearai-agent-modes", async () => {
			const settingsModes = [{ slug: "mode1", name: "Mode 1", roleDefinition: "Role 1", groups: ["read"] }]
			...
			// Should fall back to settings modes when .pearai-agent-modes is invalid
			expect(modes).toHaveLength(1)
			expect(modes[0].slug).toBe("mode1")
		})
	...
	describe("updateCustomMode", () => {
		it("should update mode in settings file while preserving .pearai-agent-modes precedence", async () => {
			...
			// Should update global state with merged modes where .pearai-agent-modes takes precedence
			expect(mockContext.globalState.update).toHaveBeenCalledWith(
				"customModes",
				expect.arrayContaining([
					expect.objectContaining({
						slug: "mode1",
						name: "Roomodes Mode 1", // .pearai-agent-modes version should take precedence
						source: "project",
					}),
				]),
			)
		...
		it("creates .pearai-agent-modes file when adding project-specific mode", async () => {
			...
			// Mock .pearai-agent-modes to not exist initially
			...
			// Verify .pearai-agent-modes was created with the project mode
			...
			// Verify the content written to .pearai-agent-modes
